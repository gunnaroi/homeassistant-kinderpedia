...
_TRANSLATION_CACHE_KEY = f"{DOMAIN}_translations"
...
class KinderpediaCalendar(KinderpediaChildEntity, CalendarEntity):
    ...
    def _build_school_event(self, event_date: date, day_info: dict) -> CalendarEvent | None:
        """Return the timed school-day event, or None if there is nothing to show."""
        translations = _load_translations(self._hass)
        ...
        for meal in ("breakfast", "lunch", "snack"):
            ...
            meal_label = translations.get(meal, meal.capitalize())
            escaped_items = ", ".join(escape(item) for item in items)
            description_parts.append(f"{icon} {meal_label}{pct_str}: {escaped_items}")

        checkin_time = self._parse_checkin_time(day_info.get("checkin", "unknown"))
        ...
        return CalendarEvent(
            summary=translations.get("school", "School"),
            start=start,
            end=end,
    ...
    def _build_nap_event(self, event_date: date, nap_text: str) -> CalendarEvent | None:
        ...
        translations = _load_translations(self._hass)
        return CalendarEvent(
            summary=translations.get("nap", "Nap"),
            start=nap_start,
            end=nap_end,
        )
